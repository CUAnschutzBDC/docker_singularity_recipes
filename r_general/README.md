...
This directory contains the recipe to build the docker container for R analysis for general analysis. This uses the bioconductor docker as a base so you can port to using `Rstudio` in the browser.

## Downloading the image
To download, the image, you can simply run

```bash
docker pull kwellswrasman/r_general:v1
```
...
```bash
docker build r_general:v1 ./
```

3. I next started an interactive shell

```bash
docker run -it --mount type=bind,source="$(pwd)",target=/home/rstudio/r_docker r_general:v1 sh
```
